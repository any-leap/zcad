...
ZCAD 不是另一个 AutoCAD 克隆。我们重新思考了 CAD 应该是什么：

| AutoCAD 思维 | ZCAD 思维 |
|-------------|----------|
| 几何是一切 | 几何是载体，**数据和关系**才是核心 |
| 实体 = 形状 | 实体 = **带属性的对象** |
| 画完即结束 | 支持**查询、统计、分析、自动化** |
| 封闭格式锁定用户 | 开放格式，**内核一致性 > 格式兼容性** |

### 核心理念

- **Entity as Object**：每个实体不只是形状，而是带有属性、标签、关系的完整对象
- **层级结构**：支持 parent/children 关系，自然支持块、组、外部参照
- **开放属性系统**：任意键值属性，不预设固定 schema
- **可查询**：所有数据都可以被查询、统计、导出
- **规则引擎**：支持条件检查和自动化处理
