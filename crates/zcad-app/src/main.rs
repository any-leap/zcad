...
impl ZcadApp {
    ...
    fn draw_dimension(&self, painter: &egui::Painter, rect: &egui::Rect, dim: &Dimension, color: Color) {
        ...
        match dim.dim_type {
            DimensionType::Aligned | DimensionType::Linear => {
                ...
                // 标注样式参数（单位：世界坐标）
                let dim_scale = 1.0 / self.camera_zoom; // 根据缩放调整
                ...
                painter.line_segment([self.world_to_screen(ext_start_p1, rect), self.world_to_screen(ext_end_p1, rect)], stroke);
                painter.line_segment([self.world_to_screen(ext_start_p2, rect), self.world_to_screen(ext_end_p2, rect)], stroke);
            ...
            DimensionType::Radius => {
                ...
                let arrow_s = self.world_to_screen(arrow_pos, rect);
                let text_s = self.world_to_screen(text_pos, rect);
                ...
                // 绘制箭头（指向圆弧）
                self.draw_arrow(painter, center_s, arrow_s, stroke);
    ...
    fn draw_dimension_text(&self, painter: &egui::Painter, rect: &egui::Rect, world_pos: Point2, text: &str, height: f64, color: egui::Color32, angle: f32) {
        let font_id = egui::FontId::proportional((height * self.camera_zoom).max(8.0) as f32);
    ...
    fn handle_left_click(&mut self) {
        ...
        match &self.ui_state.edit_state {
            EditState::Idle => match self.ui_state.current_tool {
                ...
                DrawingTool::None => {}
                DrawingTool::Text => {
                    self.ui_state.status_message = "文本功能暂未实现".to_string();
                }
                DrawingTool::Dimension => {
                    self.ui_state.edit_state = EditState::Drawing {
                    ...
                    self.ui_state.status_message = "标注: 指定第二点:".to_string();
                }
                DrawingTool::DimensionRadius => {
                    // 查找点击位置的圆或圆弧
                    ...
                    if let Some(entity) = hits.iter().find(|e| {
                        ...
                        self.ui_state.status_message = "半径标注: 指定文字位置:".to_string();
                    } else {
                ...
                DrawingTool::DimensionDiameter => {
                    ...
                    if let Some(entity) = hits.iter().find(|e| {
                        ...
                        self.ui_state.status_message = "直径标注: 指定文字位置:".to_string();
                    } else {
                        self.ui_state.status_message = "请选择圆或圆弧".to_string();
                    }
                }
                ...
                match tool {
                    ...
                    DrawingTool::Dimension => {
                        if new_points.len() == 2 {
                            ...
                            self.add_entity_with_history(entity, "创建标注");
                            self.ui_state.edit_state = EditState::Idle;
                    ...
                    DrawingTool::DimensionRadius => {
                        ...
                        if new_points.len() >= 3 {
                            ...
                            dim.dim_type = DimensionType::Radius;
                            let entity = Entity::new(Geometry::Dimension(dim));
                            self.add_entity_with_history(entity, "创建半径标注");
                            self.ui_state.edit_state = EditState::Idle;
                    ...
                    DrawingTool::DimensionDiameter => {
                        ...
                        if new_points.len() >= 3 {
                            ...
                            dim.dim_type = DimensionType::Diameter;
                            let entity = Entity::new(Geometry::Dimension(dim));
                            self.add_entity_with_history(entity, "创建直径标注");
                            self.ui_state.edit_state = EditState::Idle;
